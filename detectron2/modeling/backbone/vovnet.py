...
def conv7x7(
    in_channels, out_channels, module_name, postfix, stride=1, groups=1, kernel_size=7, padding=3
):
    """3x3 convolution with padding"""
    return [
        (
...
def conv5x5(
    in_channels, out_channels, module_name, postfix, stride=1, groups=1, kernel_size=5, padding=2
):
    """3x3 convolution with padding"""
    return [
        (
...
class VoVNet(Backbone):
    def __init__(self, cfg, input_ch, out_features=None):
        ...
        stem = conv7x7(input_ch, stem_ch[0], "stem", "1", 2)
